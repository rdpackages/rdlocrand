...
program define rdwinselect, rclass sortpreserve
	...
	tempvar treated runvar Wid Wlength dups
	...
	if r(max)>1{
		...
		qui levelsof `runvar' if `treated'==0 & `touse', local(mpvec_left)
		local mp_left = r(r)
		qui levelsof `runvar' if `treated'==1 & `touse', local(mpvec_right)
		local mp_right = r(r)
		if "`wmasspoints'"!=""{	
			mat mpvec_left = J(`mp_left',1,.)
			local k = 1
			foreach mp of numlist `mpvec_left'{
				mat mpvec_left[`k',1] = `mp'
				local k= `k'+1
			}
			mat mpvec_right = J(`mp_right',1,.)
			local k = 1
			foreach mp of numlist `mpvec_right'{
				mat mpvec_right[`k',1] = `mp'
				local k= `k'+1
			}
			local nmax = min(max(`mp_left',`mp_right'),`nwindows')
			mat wlist = J(2,`nmax',.)
		}
	}
...
********************************************************************************
	...
	if "`wmasspoints'"==""{	
		if "`obsstep'"!=""{
			...
			qui count if `runvar'>=-float(`wmin') & `runvar'<0 & `touse'
			local posl = `N_control' - r(N)
			qui count if `runvar'>=0 & `runvar'<=float(`wmin') & `touse'
			local posr = `N_control' + 1 + r(N)
			
			mata: rdlocrand_findwobs(`wobs',`nwindows'-1,`posl',`posr',"`runvar'","`dups'")
			mat wlist[1,1] = `wmin'
		}
		
		local nmax = min(`nwindows',colsof(wlist))
...
********************************************************************************
	...
	forvalues j=1/`nmax' {
		...
		if "`wmasspoints'"==""{
			...
			local tmpu = min(`j',`mp_right')
			local wupper = mpvec_right[`tmpu',1]
			local tmpl = max(1,`mp_left'+1-`j')
			local wlower = mpvec_left[`tmpl',1]
			mat wlist[1,`j'] = `wlower'
			mat wlist[2,`j'] = `wupper'
			local w = max(abs(`wupper'-`cutoff'),abs(`cutoff'-`wlower'))
		}
		...
		* Number of treated and controls 
		...
		if `nt'==0 | `nc'==0{
			di as error "insufficient observations for w = `w'"
			exit 2000
		}
		...
		* Display results
		...
		if "`covariates'"!=""{
			if "`statistic'"!="hotelling"{
				...
				output_line `w' minp `xminp' `pbin' `nt' `nc'
			}
			else {
				mat Results[`j',1] = `pval_h'
				output_line `w' `pval_h' `xminp' `pbin' `nt' `nc'
			}
		}
		else {
			mat Results[`j',1] = .
			output_line `w' . `xminp' `pbin' `nt' `nc'
		}
	...
	if "`covariates'"!=""{
		...
		egen `rowmiss' = rowmiss(`covariates')
		qui sum `rowmiss'
		if r(max)>0{
...
********************************************************************************
	...
	if "`genvars'"!=""{
		...
		qui gen double _wlength = `Wlength'
	}
...
********************************************************************************
	...
	return matrix wlist = wlist
	
	return scalar nwindows = `nmax'
	capture return scalar wmin = `wmin'
