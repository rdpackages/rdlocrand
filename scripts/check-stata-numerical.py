...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run local Stata numerical regression checks for rdlocrand.")
    parser.add_argument("--stata", help="Path to the Stata executable.")
    args = parser.parse_args()

    repo_root = find_repo_root(Path.cwd())
    stata = find_stata(args.stata)
    do_file = repo_root / "scripts" / "check-stata-numerical.do"
    log_file = repo_root / "tmp" / "stata-check" / "check-stata-numerical.log"
    ...
    if result.returncode != 0 or "Stata numerical baseline checks passed." not in log_text:
